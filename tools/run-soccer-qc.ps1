...
$testClass = "com.brecksvillelabs.scorer.SoccerRoundRobinQcTest"
$remoteArtifacts = "/sdcard/Download/scorer-qc/soccer-reference"
$timestamp = Get-Date -Format "yyyyMMdd-HHmmss"
$outputDir = Join-Path $repoRoot "android\qc-output\$timestamp-soccer"
...
try {
    ...
    Write-Host "Removing stale Soccer QC screenshots from the emulator..."
    & $adb shell "rm -rf '$remoteArtifacts'"

    Write-Host "Running Soccer round-robin QC..."
    ...
    Write-Host "Pulling Soccer QC artifacts..."
    ...
    if ($testExit -ne 0) {
        throw "Soccer QC failed. The available screenshots/test reports were still pulled for inspection."
    }
    if ($pullExit -ne 0) {
        throw "Soccer QC passed, but artifact pull failed."
    }

    Write-Host ""
    Write-Host "SOCCER QC PASSED"
}
finally {
    Pop-Location
}
